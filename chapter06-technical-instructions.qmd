...
This chapter provides selected **operational instructions** for running, configuring, and monitoring the BPA Lab demonstration factory. While the preceding chapters describe *what* the BPA Lab is, this chapter describes *how* to configure anbd use it.
...
::: {.callout-important}
## Prerequisites
The instructions assume that the BPA Lab system has been installed according to the README of the [bpa_lab_demonstration_factory repository](https://github.com/BpaLabTHCologne/bpa_lab_demonstration_factory). Docker and Docker Compose must be installed and operational on the host.
:::
...
**3.** Wait a few seconds, then execute the next user task **"Create production order"** in the same way as in step 2.

**4.** Wait for the next user task **"Place order"** and assign it to yourself. **This is the key step where the test and the productive environment differ:**

- **Test environment:** any colour (blue, red, or white) can be selected in *"Werkstück auswählen"*. Complete the user task.

- **Productive environment:** before completing the *"Place order"* user task, any workpiece (blue, red, or white) must be physically transferred to the factory so that it can be stored first. Verify on the touchscreen of the TXT controller that the factory has stored the workpiece (the corresponding workpiece must be displayed as available). Then select the corresponding colour in *"Werkstück auswählen"* and complete the user task. The factory will now run its physical simulation of the production process. **Wait until the factory has finished** before proceeding to step 5.

**5.** Execute the next user task **"Show order"** in the same way.

**6.** Execute the next user task **"Check production order"**.

**7.** Execute the next user task **"Check shipping information"**.

**8.** Execute the final user task **"Check information"**. The end-to-end process instance should now be completed, and Operate should no longer show any running instances.
...
![Search for *Index Management* in Kibana](images/wiki/01-kibana-index-management-search.png){#fig-cfg-01 fig-align="center"}

**3.** Click on the **Index Templates** tab.

![Open the *Index Templates* tab](images/wiki/02-kibana-index-templates-tab.png){#fig-cfg-02 fig-align="center"}

**4.** Enter **`operate-flownode`** in the search box and click the corresponding index template.

![Search for `operate-flownode`](images/wiki/03-kibana-operate-flownode-search.png){#fig-cfg-03 fig-align="center"}

**5.** A side window opens. Click on **Manage**, then on **Edit**.

![Open the template for editing via *Manage → Edit*](images/wiki/04-kibana-template-manage-edit.png){#fig-cfg-04 fig-align="center"}

**6.** Click on **Mappings**.

![Switch to the *Mappings* step](images/wiki/05-kibana-mappings-tab.png){#fig-cfg-05 fig-align="center"}

**7.** Enter **`date`** in the search box and edit the **`endDate`** field.

![Search for `date` and edit the date fields](images/wiki/06-kibana-date-search-endDate.png){#fig-cfg-06 fig-align="center"}

**8.** A side window opens. **Disable** the *Set format* toggle and click **Update**.

![Disable *Set format* and click *Update*](images/wiki/07-kibana-endDate-set-format-update.png){#fig-cfg-07 fig-align="center"}
...
![Review and save the updated template](images/wiki/08-kibana-review-save-template.png){#fig-cfg-08 fig-align="center"}
...
![Return to *Indices* and search for `operate-flownode`](images/wiki/09-kibana-back-to-indices.png){#fig-cfg-09 fig-align="center"}

**12.** A side window opens. Click on **Manage**, then on **Delete index**.

![Delete the existing `operate-flownode` index](images/wiki/10-kibana-delete-index.png){#fig-cfg-10 fig-align="center"}
...
![Skip the password update](images/wiki/11-grafana-skip-password.png){#fig-cfg-11 fig-align="center"}

**4.** Open the side menu in the upper-left corner. Click on **Connections**, then on **Add new connection**.

![Open *Connections → Add new connection*](images/wiki/12-grafana-add-new-connection.png){#fig-cfg-12 fig-align="center"}

**5.** Search for **`Trino`** in the search bar and click on the result.

![Search for the Trino connector](images/wiki/13-grafana-trino-connection.png){#fig-cfg-13 fig-align="center"}
...
![Open *Connections → Data sources*](images/wiki/14-grafana-data-sources.png){#fig-cfg-14 fig-align="center"}

**8.** Click **Add new data source** in the upper-right corner.

**9.** Search for **`Trino`** and click on it.

![Add a new Trino data source](images/wiki/15-grafana-add-trino-data-source.png){#fig-cfg-15 fig-align="center"}
...
![Configure the Trino data source](images/wiki/16-grafana-trino-configure.png){#fig-cfg-16 fig-align="center"}

**11.** Open the side menu and click **Dashboards**.

![Open *Dashboards* in the side menu](images/wiki/17-grafana-dashboards-menu.png){#fig-cfg-17 fig-align="center"}

**12.** Click **New → Import**.

![Import a dashboard](images/wiki/18-grafana-new-import.png){#fig-cfg-18 fig-align="center"}
...
![Edit a panel via the panel menu](images/wiki/19-grafana-panel-edit.png){#fig-cfg-19 fig-align="center"}

**15.** Click **Refresh** — the error icon disappears. Each panel of each dashboard must be refreshed once in this manner.

![Refresh the panel](images/wiki/20-grafana-panel-refresh.png){#fig-cfg-20 fig-align="center"}
...
![Prototype of the BPA Lab data architecture](images/wiki/21-data-architecture-prototype.png){#fig-data-arch fig-align="center"}
...
![Data sources, categories, and data objects in the BPA Lab](images/wiki/22-data-overview.png){#fig-data-overview-tech fig-align="center"}
...
![Skip the password update](images/wiki/11-grafana-skip-password.png){#fig-use-skip fig-align="center"}
...
![Open the *Dashboards* area](images/wiki/17-grafana-dashboards-menu.png){#fig-use-dashboards fig-align="center"}
