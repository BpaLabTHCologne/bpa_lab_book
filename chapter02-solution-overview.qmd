...
This separation is inspired by the bounded contexts familiar from DDD and makes the system easier to reason about, both for teachers explaining it and for students extending it.

The DDD concept is obviously not implemented to full extend, since the BPA Lab uses a central Business Process Management System instance (running  separated processes communicated via messages) to orchestrate the entire process and a central database to ensure data persistence.
...
If you are setting up the BPA Lab for the first time, follow the installation guide in ([Technical Instructions](chapter06-technical-instructions.qmd)). 
:::
