...
- explain why the BPA Lab implements its scenario as a tangible, end-to-end system rather than as an abstract example,
- name the three architectural layers of the BPA Lab and describe their responsibilities,
...
A demonstration factory bridges this gap. The customer order described in the previous chapter does not stop at a process diagram: it arrives via a web form, triggers a workflow in Camunda 8, dispatches commands via MQTT to physical hardware, and eventually leads to a real, manufactured product being moved through a real, physical warehouse. The architecture described in the remainder of this chapter is what makes this end-to-end execution possible.
